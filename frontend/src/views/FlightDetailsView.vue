    ...
    <div v-else-if="flight" class="flight-details-card">
      ...
      <div class="flight-image-wrapper" v-if="flight.image_url">
        <img
          ...
          :alt="`Imagen del vuelo ${flight.flight_number}`"
          class="flight-image-details"
        />
      </div>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="offering in flight.offerings" :key="offering.id" class="offering-row">
                ...
                <td>
                  <span class="price-value">u$s {{ parseFloat(offering.price).toFixed(2) }}</span>
                </td>
                <td>{{ offering.seats_available }}</td>
                <td class="class-description-cell">
                  {{ offering.flightClass?.description || '-' }}
                </td>
                <td>
                  <button
      ...
      <div class="details-actions global-actions">
        <router-link to="/flights" class="button action-button is-light"
          >Volver a Vuelos</router-link
        >
      </div>
    </div>
...
onMounted(async () => {
  ...
  try {
    loading.value = true
    error.value = ''
    const response = await getFlights({ id: flightId })
    // --- FIX: Buscar el vuelo correcto por ID ---
    if (Array.isArray(response.data)) {
      flight.value = response.data.find((f) => f.id === flightId) || response.data[0]
    } else {
    ...
    error.value =
      err.response && err.response.status === 404
        ? 'Vuelo no encontrado.'
        : 'No se pudo cargar detalles del vuelo.'
    flight.value = null
  } finally {
    loading.value = false
  }
})
const selectOfferingAndProceed = (offering) => {
  router.push({
    name: 'create-booking',
    params: { flightId: flight.value.id },
    query: { offeringId: offering.id },
  })
}
const redirectToLogin = (offering) => {
  router.push({
    name: 'login',
    query: { redirect: router.currentRoute.value.fullPath, offeringId: offering.id },
  })
}
</script>
...
.flight-details-container {
  ...
  margin: 30px auto;
  padding: 0 15px;
  /* Padding lateral para la vista general */
}
...
/* Padding base para secciones del cuerpo */
.general-info-body {
  border-bottom: 1px solid #f0f0f0;
}
...
.details-body h3 {
  /* "Horarios", "Selecciona tu Clase" */
  font-size: 1.25em;
  color: #4a4a4a;
  margin-top: 0;
  margin-bottom: 18px;
  padding-bottom: 8px;
  /* No más borde inferior aquí, se maneja por sección */
  font-weight: 600;
}
...
.offerings-table td {
  ...
  /* Alinear verticalmente en el medio */
}
...
.compact-button {
  ...
  white-space: nowrap;
  /* Evitar que el texto del botón se parta */
}
